    ...
    <section class="content">
        <div class="row">
                <div class="col-sm-2 pull-right" style="padding-bottom: 10px;">
                    <a href="{{route('homePageSlider.create')}}"><button type="button" class="btn btn-block btn-primary">New Home Page Slider</button></a>
                </div>
            <div class="col-xs-12">
                <div class="box">
                    <!-- /.box-header -->
...
<script>
    ...
    $(document.body).on('click', '.actStatus' ,function(event){
        ...
        bootbox.confirm({
        message: "Are you sure you want to change Home Page Slider status ?",
            buttons: {
            'cancel': {
                label: 'No',
            ...
            callback: function(result){
                if (result){
                    ...
                    $.ajax({
                        type :'POST',
                        data : {id:dbid},
                        url  : 'homePageSlider/status-change',
    ...
    function deleteConfirm(id){
        ...
        message: "Are you sure you want to delete ?",
            ...
            callback: function(result){
                if (result){
                    $.ajax({
                        url: SITE_URL + '/admin/homePageSlider/'+id,
                        type: "DELETE",
                        cache: false,
                        data:{ _token:'{{ csrf_token() }}'},
                        success: function (data , textStatus, xhr) {
                            if(data== true && textStatus=='success' && xhr.status=='200'){
                                toastr.warning('Home Page Slider Deleted !!');
